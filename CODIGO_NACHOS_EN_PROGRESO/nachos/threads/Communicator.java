...
public class Communicator {
    ...
    public void speak(int word){
        lock.acquire();
        Lib.debug(dbgCommunicator, "*Transmitiendo '" + word + "'");
        ...
        while (transferenceInProgress || (this.listenerCalls == 0)){
            if (this.listenerCalls == 0)
                Lib.debug(dbgCommunicator, "Durmiendo thread " + KThread.currentThread() + " esperando a que un listener sea llamado...");
            if (transferenceInProgress)
            Lib.debug(dbgCommunicator, "Durmiendo thread " + KThread.currentThread() + " esperando a que se termine la transferencia actual...");
    ...
    public int listen() {
        lock.acquire();
        Lib.debug(dbgCommunicator, "*Escuchando...");
        this.listenerCalls++;           // Se ha llamado el métodos listen, incrementar listenerCalls

        if (this.speakerCalls != 0)
            Lib.debug(dbgCommunicator, "Listener ha sido llamado, despertando siguiente speaker en cola de espera...");
        ...
        while (!transferenceInProgress){
            Lib.debug(dbgCommunicator, "Durmiendo thread " + KThread.currentThread() + " esperando a que un speaker haga la transferencia");
