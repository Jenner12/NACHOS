...
public class Condition2 {
    ...
    public void sleep() {
        ...
        Lib.debug('c', "Sleep Begin: El lockHolder actual era: " + this.conditionLock.getLockHolder());
    ...
    public void wake() {
        Lib.assertTrue(conditionLock.isHeldByCurrentThread());
        Lib.debug('c', "Wake Begin: El lockHolder actual era: " + this.conditionLock.getLockHolder());
        boolean intStatus = Machine.interrupt().disable();
        KThread kt;
        if (! ThreadQueueMethods.ThreadQueueIsEmpty(this.blockedThreadsQueue)){
            kt = blockedThreadsQueue.nextThread();
            kt.ready();
            Lib.debug('c', "Se desperto al thread: " + kt);
        }
        Machine.interrupt().restore(intStatus);
    }
    ...
    public void wakeAll() {
        Lib.assertTrue(conditionLock.isHeldByCurrentThread());
        Lib.debug('c', "WakeAll Begin: El lockHolder actual era: " + this.conditionLock.getLockHolder());
        boolean intStatus = Machine.interrupt().disable();
    ...
    private ThreadQueue blockedThreadsQueue = ThreadedKernel.scheduler.newThreadQueue(false); //viejo
}
